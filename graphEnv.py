...
class TwoAgentsEnv(gym.Env):
    ...
    def __init__(self, grid_size: int, graph_number:int = 0, plot=False):
        ...
        self.agent2_position: Tuple[int, int] = (4, 4)
    ...
    def make_graph(self) -> nx.Graph:
        ...
        if self.graph_number ==0:
            ...
            self.start_agent2_position = (4, 4)
            grid[2, 0] = 1
            grid[2, 3] = 1
            grid[2, 2] = 1
            grid[3, 3] = 1
    ...
    def step(self, action1: int, action2: int) -> Tuple[int, int, int, int, bool, bool, Dict]:
        ...
        if self.agent1_position == self.agent2_position:
            rewards[0] -= 0.5
            rewards[1] -= 0.5
            # Optional: Move agents back to previous positions
            # self.agent1_position = prev_pos1
            # self.agent2_position = prev_pos2
        ...
        return self._get_observation(agent_number=0), self._get_observation(agent_number=1), rewards[0], rewards[1], done, _, _ 
    ...
    def _get_observation3(self, agent_number) -> int:
        if agent_number == 0:
            agent = self.agent1_position
        else:
            agent = self.agent2_position
    ...
    def _get_observation3(self, agent_number):
        ...
        if agent_number == 0:
            ...
            opponent_pos = self.agent2_position
        else:
            agent_pos = self.agent2_position
            opponent_pos = self.agent1_position
            
        current_node = self.xy_pos_to_int(*agent_pos)
        opponent_node = self.xy_pos_to_int(*opponent_pos)
        ...
        collision_bit = 1 if current_node == opponent_node else 0
        
        return (current_node * 32) + (bitmask << 1) + collision_bit


def plot_rewards(rewards1, rewards2, eps, roll):
...
if __name__ == "__main__":
    ...
    plot_rewards(rewards1, rewards2, eps, 30)
    ...
    from twoQtableHelper import plot_graph
